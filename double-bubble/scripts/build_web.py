...
<body>
    ...
    <div class="card">
      ...
      <div class="controls">
        <div class="field wide">
          ...
          <div class="hint">Each row should be a single employee + date. Hour columns hold 0–1 hours for that slot (e.g., 0.5 = 30m). Legacy start/end rows or ISO timestamps remain supported.</div>
        </div>
        <div class="field wide">
  ...
  const SCHEDULED_TYPES = new Set(["scheduled","reg","regular"]);
  const CALLIN_TYPES = new Set(["call-in","callin","ot2","ot"]);
  const normalizeType = (value) => (value ?? "").toString().trim().toLowerCase();
  const isScheduledType = (value) => SCHEDULED_TYPES.has(normalizeType(value));
  const isCallInType = (value) => CALLIN_TYPES.has(normalizeType(value));
  ...
  function expandHourlyRow(row){
    ...
    if (!emp || !day) return [];
    const baseRaw = {...row};
    const typ = normalizeType(row.shift_type ?? row.type ?? row.Type ?? "") || "scheduled";
    const segs = [];
    let current = null;
  ...
  function normalizeRows(rows){
    const out = [];
    for (const r of rows){
      const hourly = expandHourlyRow(r);
      if (hourly){
        out.push(...hourly);
        continue;
      }
      ...
      const typ = normalizeType(r.shift_type ?? r.type ?? r.Type ?? "") || "scheduled";
  ...
  document.querySelector("#loadSampleBtn").addEventListener("click", ()=>{
    ...
    const header = ["employee_id","calendar_date", ...hourCols, "shift_type","crew","district","job_class"];
    const sampleRows = [
      {
        employee_id:"E001",
    ...
    afterLoad();
    document.querySelector("#loadStatus").textContent = `Loaded demo sample (${rawRows.length} rows; hourly grid).`;
  });
  ...
  function afterLoad(){
    ...
    const known = new Set(["employee_id","start_datetime","end_datetime","shift_type","Employee","Start","End","start","end","type","calendar_date","calendarDate","CalendarDate","date"]);
