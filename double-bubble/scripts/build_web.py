...
<body>
  ...
  <main>
    <div class="card">
      ...
      <div class="controls">
        <div class="field wide">
          <label>Upload CSV (columns: <span class="kbd">employee_id,start_datetime,end_datetime</span>, optional: <span class="kbd">shift_type, crew, district, job_class</span>)</label>
          <input type="file" id="fileInput" accept=".csv" />
          <div class="hint">Timestamps like <span class="kbd">2025-08-10 07:00</span> or ISO-8601 are supported. Or paste a CSV URL and click “Fetch CSV”.</div>
        </div>
        <div class="field wide">
    ...
    <div class="card">
      ...
      <div class="hint-row">
        ...
        <div class="legend">
          ...
          <span class="swatch callin"></span> Call-in (dashed outline)
    ...
    <div class="card">
      ...
      <ul class="note">
        <li>Double‑bubble eligibility is attributed to the <em>current shift</em> if the rest gap from the previous shift’s end to this shift’s start is below the threshold.</li>
  ...
  function normalizeRows(rows){
    ...
    for (const r of rows){
      ...
      const typ = (r.shift_type ?? r.type ?? r.Type ?? "").toString().trim().toLowerCase();
      if (!emp || !st || !en) continue;
      if (en < st){
        const en2 = addDays(en, 1);
        out.push({employee_id: emp, start: st, end: en2, shift_type: typ || "scheduled", raw:r});
      } else {
        out.push({employee_id: emp, start: st, end: en, shift_type: typ || "scheduled", raw:r});
      }
  ...
  function computeRestAndFlags(arr){
    ...
    for (const s of list){
      ...
      const prev = prevByEmp.get(key);
      s.prev_end = prev || null;
      s.rest_gap_h = prev ? hoursBetween(prev, s.start) : null;
      s.double_bubble = (s.rest_gap_h != null) ? (s.rest_gap_h < params.restThreshold) : false;
      prevByEmp.set(key, s.end);
    }
    return list;
  }
  ...
  function perEmployeeBaseline(arr){
    ...
    for (const [emp, list] of byEmp){
      const pool = (params.baselineMode === "scheduled")
        ? list.filter(x=> (x.shift_type||"").toLowerCase()==="scheduled")
        : list.slice();
  ...
  function renderOverlayForEmployees(empIds){
    ...
    empIds.forEach((empId, idx) => {
      ...
      for (const s of segs){
        ...
        r.setAttribute("stroke", s.deviation ? "#ffb648" : (s.shift_type==="call-in" ? "#a0a4ae" : "rgba(0,0,0,0)"));
        r.setAttribute("stroke-width", s.shift_type==="call-in" ? "1.5" : (s.deviation ? "1.5":"0"));
        r.setAttribute("stroke-dasharray", s.shift_type==="call-in" ? "4 4" : (s.deviation ? "0" : "0"));
        if (s.double_bubble) r.setAttribute("fill", "url(#hatch)");

        const durH = hoursBetween(s.start, s.end);
        const callin = (s.shift_type||"") === "call-in";
        const tip = `
  ...
  function renderFlagTable(flagged){
    ...
    for (const f of flagged){
      ...
      tr.innerHTML = `
        ...
        <td>${(f.shift_type||"")==="call-in"?'<span class="pill ok">Yes</span>':'<span class="pill">No</span>'}</td>
  ...
  document.querySelector("#fileInput").addEventListener("change", async (e)=>{
    ...
    try{
      ...
      afterLoad();
      document.querySelector("#loadStatus").textContent = `Loaded ${rawRows.length} rows from file.`;
    }catch(err){
      console.error(err);
  ...
  document.querySelector("#fetchBtn").addEventListener("click", async ()=>{
    ...
    try{
      ...
      afterLoad();
      document.querySelector("#loadStatus").textContent = `Loaded ${rawRows.length} rows from URL.`;
    }catch(err){
      console.error(err);
      document.querySelector("#loadStatus").textContent = "Error fetching: " + err.message;
    }
  });
  document.querySelector("#loadSampleBtn").addEventListener("click", async ()=>{
    const sample = `employee_id,start_datetime,end_datetime,shift_type,crew,district,job_class
E001,2025-08-11 07:00,2025-08-11 15:30,scheduled,A,North,Lineman
E001,2025-08-11 19:30,2025-08-11 22:30,call-in,A,North,Lineman
E001,2025-08-12 07:00,2025-08-12 15:30,scheduled,A,North,Lineman
E001,2025-08-13 07:00,2025-08-13 15:30,scheduled,A,North,Lineman
E001,2025-08-13 20:00,2025-08-14 00:30,call-in,A,North,Lineman
E001,2025-08-14 07:00,2025-08-14 15:30,scheduled,A,North,Lineman
E001,2025-08-15 07:00,2025-08-15 15:30,scheduled,A,North,Lineman
E001,2025-08-16 08:00,2025-08-16 14:00,call-in,A,North,Lineman
E002,2025-08-11 07:00,2025-08-11 15:30,scheduled,A,North,Lineman
E002,2025-08-12 07:00,2025-08-12 15:30,scheduled,A,North,Lineman
E002,2025-08-12 23:45,2025-08-13 03:15,call-in,A,North,Lineman
E002,2025-08-13 07:00,2025-08-13 15:30,scheduled,A,North,Lineman
E002,2025-08-14 07:00,2025-08-14 15:30,scheduled,A,North,Lineman
E002,2025-08-14 21:30,2025-08-15 01:00,call-in,A,North,Lineman
E002,2025-08-15 07:00,2025-08-15 15:30,scheduled,A,North,Lineman
E002,2025-08-15 20:00,2025-08-15 22:00,call-in,A,North,Lineman
E002,2025-08-16 08:00,2025-08-16 12:00,call-in,A,North,Lineman
E003,2025-08-11 06:30,2025-08-11 15:00,scheduled,B,South,Lineman
E003,2025-08-13 06:30,2025-08-13 15:00,scheduled,B,South,Lineman
E003,2025-08-14 06:30,2025-08-14 15:00,scheduled,B,South,Lineman
E003,2025-08-15 06:30,2025-08-15 14:30,scheduled,B,South,Lineman
E003,2025-08-15 23:00,2025-08-16 06:00,call-in,B,South,Lineman
E003,2025-08-16 12:00,2025-08-16 20:00,call-in,B,South,Lineman
E004,2025-08-11 07:00,2025-08-11 15:30,scheduled,B,South,Apprentice
E004,2025-08-14 07:00,2025-08-14 15:30,scheduled,B,South,Apprentice
E004,2025-08-16 07:00,2025-08-16 15:30,scheduled,B,South,Apprentice`;
    rawRows = parseCSV(sample);
    afterLoad();
    document.querySelector("#loadStatus").textContent = `Loaded demo sample (${rawRows.length} rows).`;
  });
  ...
  function afterLoad(){
    ...
    const known = new Set(["employee_id","start_datetime","end_datetime","shift_type","Employee","Start","End","start","end","type"]);
    ...
    // employees list
    employees = Array.from(new Set(rawRows.map(r=> (r.employee_id ?? r.Employee ?? "").toString()))).sort();
    ...
    const dates = rawRows.map(r=> parseMaybe(r.start_datetime ?? r.start ?? r.Start ?? null)).filter(Boolean).sort((a,b)=>a-b);
    if (dates.length){
  ...
  function recomputeAll(){
    ...
    // 3) Rest + flags
    let computed = computeRestAndFlags(ranged);
    // 4) Baseline
